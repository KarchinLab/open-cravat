...
(function($) {
	...
	fni._create = function() {
		...
		that.iGroupView = new _pq.cGroupView(that);
		if (_pq.cMerge) {
	...
	fn.loadState = function(ui) {
		...
		var self = this,
			...
			jextend = $.extend,
			id = $grid[0].id,
			state = ui.state || ((typeof(Storage) === "undefined") ? undefined : localStorage.getItem("pq-grid" + (id ? id : "")));
		if (!state) {
			return false
		} else {
	...
	fn.reset = function(ui) {
		...
		if (group) {
			...
			groupModel.refresh = false;
			self.groupOption(groupModel)
		}
		if (ui.refresh !== false) {
			if (filter) {
				self.filter({
...
(function($) {
	...
	_p.getXlsCols = function(CM, CMLen) {
		...
		for (var i = 0; i < CMLen; i++) {
			...
			cols.push('<col min="', col, '" max="', col, '" hidden="', hidden, '" customWidth="1" width="', (width / 7), '"/>')
		}
		cols.push("</cols>");
		return cols.join("")
	};
	...
	_p.getXlsBody = function(CM, CMLen, data, dataLen, remotePage, offset, iMerge, render, iGV) {
		...
		for (i = 0; i < dataLen; i++) {
			...
			for (j = 0; j < CMLen; j++) {
				...
				datatype = (cv == null ? null : objDataType[column.dataType]), datatype = datatype ? datatype : "String";
				if (datatype == "Number") {
					if (isNaN(cv * 1)) {
						datatype = "String"
					} else {
	...
	_pMerge.renderCell = function(ui) {
		...
		if (mc[a_r1] && (mcRec = mc[a_r1][a_ci])) {
			...
			if (!mcRec.show && !firstRowPage && !topEdge && !leftEdge && !bothEdges) {
				...
				if (fr && a_r1 < fr_off && a_r1 + rowspan > frVisible + offset) {
					var rowspan1 = rowspan - calcVisibleRows(pdata, fr, initV),
						rowspan2 = calcVisibleRows(pdata, a_r1 - offset, fr);
...
(function($) {
	...
	var cGroupView = _pq.cGroupView = function(that) {
		var self = this;
		self.that = that;
	...
	_pGroupView.onCellClick = function(self) {
		return function(evt, ui) {
			if (ui.rowData.pq_gtitle && $(evt.originalEvent.target).hasClass("pq-group-icon")) {
				self.toggle(ui.rowIndxPage)
			}
		}
	};
	_pGroupView.onChange = function(self, GM) {
		return function() {
			self.saveState(GM.refreshOnChange)
		}
	};
	_pGroupView.onCellMouseDown = function() {
		return function(evt, ui) {
			if (ui.rowData.pq_gtitle && $(evt.originalEvent.target).hasClass("pq-group-icon")) {
	...
	_pGroupView.onCellKeyDown = function(self, GM) {
		return function(evt, ui) {
	...
	_pGroupView.removeGroup = function(dataIndx, refresh) {
		var self = this,
			that = self.that,
			GM = that.options.groupModel,
			groupIndx = GM.dataIndx;
		for (var i = 0; i < groupIndx.length; i++) {
	...
	_pGroupView.addGroup = function(dataIndx, indx) {
		var that = this.that,
			GM = that.options.groupModel;
		GM.dataIndx = GM.dataIndx || [];
		if (dataIndx != null && $.inArray(dataIndx, GM.dataIndx) == -1) {
			GM.dataIndx.push(dataIndx);
			this.refreshFull()
		}
	};
	_pGroupView._initHeadSortable = function() {
		var self = this,
			that = self.that,
			$h = self.$header,
			o = that.options;
	...
	_pGroupView.onColumnDrag = function(self) {
		return function(evt, ui) {
			var col = ui.column,
				CM = col.colModel;
			if (CM && CM.length) {
	...
	_pGroupView._initHeadDroppable = function() {
		var self = this,
			that = self.that,
			$h = self.$header;
		if ($h) {
	...
	_pGroupView.onDrop = function(that, self) {
		return function(evt, ui) {
			var colIndxDrag = (ui.draggable.attr("pq-col-indx") * 1),
	...
	_pGroupView.onSortable = function(self, o) {
		return function() {
			var arr = [],
				GM = o.groupModel,
				GMDataIndx = GM.dataIndx,
	...
	_pGroupView._initHeader = function(o, GM) {
		var self = this;
		if (self.$header) {
			var $h = self.$header,
	...
	_pGroupView.createHeader = function() {
		var self = this,
			that = self.that,
			$h = self.$header,
			o = that.options,
			...
			if (len) {
				...
				for (var i = 0; i < len; i++) {
					var dataIndx = GMdataIndx[i],
						...
						dir = GM.dir[i];
					arr.push("<div class='pq-group-item' data-indx='", dataIndx, "' data-dir='", dir, "'>", "<span class='", self.toggleIcon, cicon, "' ></span>", column.pqtitle || (typeof column.title == "string" ? column.title : dataIndx), "<span class='", self.groupRemoveIcon, "' ></span></div>")
				}
	...
	_pGroupView._initHeaderMenu = function() {
		var self = this,
			that = self.that,
			BS_on = that.BS_on,
			o = that.options,
	...
	_pGroupView.setOption = function() {
		var self = this;
		if (self._init) {
			self.refreshColumns();
	...
	_pGroupView.onDataReady = function(self, that) {
		return function() {
			self.tree.length = 0;
	...
	_pGroupView.onColumnOrder = function(self) {
		return function() {
			self._initmerge()
		}
	};
	_pGroupView.pq_gc_ = function() {
		return "pq_gc_"
	};
	_pGroupView.onBeforeSort = function(self, that) {
		return function(evt) {
	...
	_pGroupView.sortType = function(tmp_di) {
	...
	_pGroupView.groupToggle = function(dataIndx) {
		var that = this.that,
			GM = that.options.groupModel,
			collapsed = GM.collapsed,
			indx = $.inArray(dataIndx, GM.dataIndx);
		collapsed[indx] = !collapsed[indx];
		this.trigger(null, indx);
		this.refreshView()
	};
	...
	_pGroupView.expandIndx = function(ui) {
		var that = this.that,
			collapsed = ui.close == true,
			indices = ui.indx.split(","),
			len = indices.length,
			tree = this.tree,
		...
		for (var l = 0; l < len; l++) {
			...
			rip = node.rip;
			if (!collapsed || (collapsed && l == len - 1)) {
				rd = data[rip];
				if (rd.pq_close != collapsed) {
					rdFinal = rd;
					rd.pq_close = collapsed
				}
			}
			offset = findOffset(tree, (l + 1), rip, indx)
		}
		if (rdFinal) {
			this.trigger(rdFinal);
			this.saveState(true)
		}
	};
	_pGroupView._editorSummary = function(o, GM) {
		...
		return function(ui) {
			...
			if (rd.pq_gsummary || rd.pq_gtitle) {
				...
				return {
					...
					getData: GM.getData || function(ui) {
	...
	_pGroupView._renderTitle = function(o, GM) {
		var that = this.that,
			BS = o.bootstrap,
			bts_on = BS.on,
		...
		return function(ui) {
			...
			if (rd.pq_gtitle && ui.cellData != null) {
				...
				level = rd.pq_level;
				title = GM.title;
				title = title[level] ? title[level] : GM.titleDefault;
	...
	_pGroupView._renderSummary = function(o) {
		var that = this.that;
		return function(ui) {
			var rd = ui.rowData,
	...
	_pGroupView.refreshColumns = function() {
		var that = this.that,
			o = that.options,
			GM = o.groupModel,
			GM_on = GM.on,
			fixCols = GM.fixCols,
			renderTitle, renderSummary, column, csummary, csummaryType, groupIndx = GM.dataIndx,
		...
		while (i--) {
			column = CM[i];
			if (column._render) {
				if (column._render == column._renderGT) {
					delete column._renderGT;
					delete column._render
				} else {
					if (column._render == column._renderGS) {
	...
	_pGroupView.refreshFull = function() {
		this.refreshColumns();
		this.that.sort()
	};
	_pGroupView.refreshView = function() {
		this.that.refreshView()
	};
	_pGroupView.expandAll = function(ui) {
		ui = ui || {};
		var that = this.that,
			collapsed = ui.close == true,
			GM = that.options.groupModel,
			...
			groupIndx = GM.dataIndx,
			i;
		for (i = 0; i < groupIndx.length; i++) {
			arr[i] = collapsed
		}
		this.trigger(null, null, true);
		this.refreshFull()
	};
	_pGroupView.showHideRows = function(initIndx, level, hide, merge) {
		var arr = [],
			that = this.that,
			data = that.pdata;
		for (var i = initIndx, len = data.length; i < len; i++) {
			var rowData = data[i];
			if (rowData.pq_gsummary) {
				if (rowData.pq_level < level) {
					break
				} else {
					rowData.pq_hidden = hide
				}
			} else {
				if (rowData.pq_gtitle) {
					var collapsed = rowData.pq_close;
					if (collapsed) {
						arr.push({
							indx: i,
					...
					if (rowData.pq_level <= level) {
						...
						rowData.pq_hidden = (merge && !collapsed) ? true : hide
					}
				} else {
					rowData.pq_hidden = hide
				}
	...
	_pGroupView.trigger = function(rd, level, all) {
		var that = this.that,
			close, GM = that.options.groupModel,
			state = {};
		if (rd) {
			var level = rd.pq_level,
				di = GM.dataIndx[level],
				val = rd[di];
			close = rd.pq_close;
			state = {
				level: level,
		...
		that._trigger("group", null, state)
	};
	_pGroupView.toggle = function(rip) {
		var that = this.that,
			pdata = that.pdata,
			rd = pdata[rip];
		if (!rd.pq_close) {
	...
	_pGroupView.saveState = function(refresh) {
		var that = this.that,
			GM = that.options.groupModel;
	...
	_pGroupView._initmerge = function() {
		var that = this.that,
			o = that.options,
		...
		for (var l = 0; l < tree.length; l++) {
			_tree = tree[l];
			ci = that.getColIndx({
				dataIndx: o.groupModel.dataIndx[l]
			});
	...
	_pGroupView._initcollapsed = function() {
		var that = this.that,
	...
	_pGroupView._groupChange = function(ignoreCase) {
		var trim = $.trim,
			pq_gc_ = this.pq_gc_();
	...
	fn.group = function(ui) {
		var iGV = this.iGroupView;
		iGV[ui.indx ? "expandIndx" : "expandAll"](ui)
	};
	fn.groupOption = function(ui) {
		var di = ui.dataIndx,
