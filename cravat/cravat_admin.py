...
class ExampleCommandsFormatter(
    object,
):
...
def list_local_modules(
    pattern=r".*", types=[], include_hidden=False, tags=[], quiet=False, raw_bytes=False
):
    if quiet:
        all_toks = []
    else:
...
def install_modules(args):
    ...
    for module_name in matching_names:
        ...
        if args.version is None:
            if local_info is not None:
                ...
                if not args.force and LooseVersion(local_ver) >= LooseVersion(
                    remote_ver
                ):
                    print(
            ...
            if (
                ...
                and LooseVersion(local_info.version) == LooseVersion(args.version)
            ):
                print(
    ...
    if args.private:
        if args.version is None:
            sys.exit(
                "--include-private cannot be used without specifying a version using -v/--version"
            )
        ...
        if not (args.yes):
            while True:
                ...
                if resp == "n":
                    ...
                    print(
                        "Your response ('{:}') was not one of the expected responses: y, n".format(
                            resp
                        )
                    )
                    continue
...
def new_annotator(args):
    ...
    print(
        "Annotator {0} created at {1}".format(
            args.annotator_name, module_info.directory
        )
    )
...
parser_install = subparsers.add_parser(
    "install", help="installs modules.", description="installs modules."
)
parser_install.add_argument(
    "modules", nargs="+", help="Modules to install. May be regular expressions."
)
parser_install.add_argument("-v", "--version", help="Install a specific version")
parser_install.add_argument(
    "-f",
    "--force",
    action="store_true",
...
# publish
parser_publish = subparsers.add_parser("publish", help="publishes a module.")
parser_publish.add_argument("module", help="module to publish")
...
parser_publish.add_argument(
    "-u", "--user", required=True, help="user to publish as. Typically your email."
)
parser_publish.add_argument(
    "-p", "--password", help="password for the user. Enter at prompt if missing."
)
parser_publish.add_argument(
    "--force-yes",
    default=False,
...
parser_create_account = subparsers.add_parser(
    "create-account", help="creates a CRAVAT store developer account."
)
parser_create_account.add_argument("username", help="use your email as your username.")
parser_create_account.add_argument("password", help="this is your password.")
parser_create_account.set_defaults(func=create_account)

# change-password
parser_change_password = subparsers.add_parser(
    "change-password", help="changes CRAVAT store account password."
)
parser_change_password.add_argument("username", help="username")
parser_change_password.add_argument("cur_pw", help="current password")
parser_change_password.add_argument("new_pw", help="new password")
parser_change_password.set_defaults(func=change_password)

# reset-password
parser_reset_pw = subparsers.add_parser(
    "reset-password", help="resets CRAVAT store account password."
)
parser_reset_pw.add_argument("username", help="username")
parser_reset_pw.set_defaults(func=send_reset_email)

# verify-email
parser_verify_email = subparsers.add_parser(
    "verify-email", help="sends a verification email."
)
parser_verify_email.add_argument("username", help="username")
parser_verify_email.set_defaults(func=send_verify_email)

# check-login
parser_check_login = subparsers.add_parser(
    "check-login", help="checks username and password."
)
parser_check_login.add_argument("username", help="username")
parser_check_login.add_argument("password", help="password")
parser_check_login.set_defaults(func=check_login)

# test input file
parser_make_example_input = subparsers.add_parser(
    "make-example-input", help="makes a file with example input variants."
)
parser_make_example_input.add_argument(
    "directory", default="", help="Directory to make the example input file in"
)
...
parser_new_annotator.add_argument("annotator_name", help="Annotator name")
parser_new_annotator.set_defaults(func=new_annotator)

# opens issue report
parser_report_issue = subparsers.add_parser(
...
parser_show_cravat_conf = subparsers.add_parser(
    "show-cravat-conf", help="shows cravat configuration."
)
parser_show_cravat_conf.set_defaults(func=show_cravat_conf)

# shows version
parser_show_version = subparsers.add_parser("version", help="shows open-cravat version")
