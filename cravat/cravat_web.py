...
def main (url=None):
    ...
    try:
        ...
        async def clean_sessions():
            ...
            """
            try:
                max_age = conf.get_system_conf().get('max_session_age',604800) # default 1 week
                interval = conf.get_system_conf().get('session_clean_interval',3600) # default 1 hr
                while True:
                ...
                if args.nostdoutexception == False:
                    traceback.print_exc()
        if servermode and server_ready:
            if 'max_session_age' in conf.get_system_conf():
